...
def get_or_build_tokenizer(config, dataset, language):
    tokenizer_path = get_tokenizer_path(config, language)
...
def get_dataset(config):
    ...
    train_dataset_raw, validation_dataset_raw = random_split(
        filtered_data, [train_dataset_size, validation_dataset_size]
    )
...
def train_model(config):
    ...
    weights_dir = get_project_root() / config["model_folder"]
    ...
    writer = SummaryWriter(get_experiment_dir(config))
    ...
    pad_src = tokenizer_src.token_to_id("[PAD]")
    loss_fn = nn.CrossEntropyLoss(ignore_index=pad_src, label_smoothing=0.1).to(device)
